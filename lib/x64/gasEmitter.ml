...
let emit_instruction = function
  ...
  | Push operand -> Printf.sprintf "%spushq %s" indent (emit_operand (operand, QWord))
  | Pop r -> Printf.sprintf "%spopq %s" indent (emit_operand (Reg r, QWord))
  | Call name ->
...
let emit_top_level = function
  ...
  | StaticVar { name; global; alignment; init_list } ->
    ...
    let emit_static_init = function
      | CharInit i | UCharInit i -> ".byte " ^ Int32.to_string i
